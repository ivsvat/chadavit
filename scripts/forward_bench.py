...
from src.backbones.vit import chada_vit, dev_chada_vit
from src.utils.demo_utils import bytes_to_giga_bytes, print_model
from src.utils.bench_utils import (
    generate_data,
    collate_images,
    tokenise,
    extract_features,
...
def main(cfg):
    ...
    logging.basicConfig(
        filename=os.path.join(cfg["LOG_DIR"], log_name + ".log"), level=logging.INFO
    )
    ...
    model = getattr(dev_chada_vit, cfg["MODEL"])(
    ...
    logger.info(
        f"max_memory_allocated: {bytes_to_giga_bytes(torch.cuda.max_memory_allocated(device))} (GB)"
    )
