...
from src.backbones.vit.layers import TransformerEncoderLayer, TokenLearner, DevTransformerEncoderLayer
...
class DevChAdaViT(nn.Module):
    """ Channel Adaptive Vision Transformer"""
    def __init__(self, img_size=[224], in_chans=1, embed_dim=192, patch_size=16, num_classes=0, depth=12,
                 num_heads=12, drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm, return_all_tokens=True, max_number_channels=10, **transformer_layer_kwargs):
        super().__init__()
        ...
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule
        self.blocks = nn.ModuleList([
            DevTransformerEncoderLayer(d_model=self.embed_dim, nhead=num_heads, dim_feedforward=2048, dropout=dpr[i], batch_first=True, **transformer_layer_kwargs)
            for i in range(depth)
        ])
        self.norm = norm_layer(self.embed_dim)
